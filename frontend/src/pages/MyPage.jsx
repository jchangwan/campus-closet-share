...
// ★ 2. Main Component (프로필 정보는 상위(App)에서, 나머지는 로컬/백엔드 연동)
export default function MyPage({ currentUser, onUpdateProfile }) {
  ...
  // (1) 내가 등록한 옷
  const myCloset = [] /* allClosetItems(API 연동 시 대체) */.filter(item => item.isMine);
  
  // (2) 내가 대여한 옷 (상태가 rented이고, 빌린 사람이 나인 경우)
  const rentedItems = [] /* allClosetItems(API 연동 시 대체) */.filter(item => 
    item.status === 'rented' && item.rentedBy === currentUser.email
  );
  const rentedItemIds = rentedItems.map(item => item.id);

  // (3) 찜한 옷
  const bookmarkedItems = [] /* allClosetItems(API 연동 시 대체) */.filter(item => 
    item.isBookmarked && !item.isMine && !rentedItemIds.includes(item.id)
  );

  const savedPosts = [] /* allSocialPosts(API 연동 시 대체) */.filter(post => post.isSaved);

  // (4) 대여 등록 모달에 보여줄 목록 (내 옷 아니고, 대여 가능한 것)
  const availableForRentItems = [] /* allClosetItems(API 연동 시 대체) */.filter(item => 
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <RentalRegistrationModal 
        ...
        onRent={(id, date) => { console.log('rent item', id, date); }}
      />
    </div>
  );
}